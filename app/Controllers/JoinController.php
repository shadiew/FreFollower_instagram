<?php
    ...
    class JoinController extends BaseController {
        ...
        function onActionExecuting() {
            ...
            if(Wow::get("project/member2LoginPrefix") != "/join" && $this->route->defaults["controller"] == "Home") {
                return $this->notFound();
            }

            if($this->logonPerson->isLoggedIn()) {
                // return $this->redirectToUrl("/tools");
                return $this->redirectToUrl("https://informatikamu.id/?go=aHR0cHM6Ly9pZy5pbmZvcm1hdGlrYW11LmlkL3Rvb2xz");
            }
        }
        ...
        function IndexAction() {
            ...
            if($this->request->method == "POST") {
                ...
                //                            "status" => "0",
                //                            "error"  => "When entering your username, select your profile from the drop-down list."
                //                        ));
                // }
                ...
                if($data) {
                    ...
                    if($objInstagram instanceof Instagram && $arrLogin["status"] == "ok") {
                        ...
                        if(Wow::get("ayar/resimsizLogin") == 1 && !stristr($userData["user"]["profile_pic_url"], "s150x150")) {
                            return $this->json(array(
                                                   "status" => "0",
                                                   "error"  => "Accounts without profile photos cannot be entered.!"
                                               ));
                        }
                ...
                if($successLogin) {
                    return $this->json(array(
                                           "status"    => "success",
                                        //   "returnUrl" => "/tools"
                                            "returnUrl" => "https://informatikamu.id/?go=aHR0cHM6Ly9pZy5pbmZvcm1hdGlrYW11LmlkL3Rvb2xz"
                                       ));
                } else {
                    return $this->json(array(
                                           "status" => "0",
