<?php
    ...
    class HomeController extends BaseController {
            ...
            if($this->request->ajax) {
                ...
                if(count($talepler) > 0) {
                    ...
                    return $this->json(array(
                                           "status" => 2,
                                           "error"  => "Yapmış olduğunuz bir talep bulunamamıtır."
                                       ));
                }
        ...
        function ApiDocsAction() {
            ...
            $this->view->set("title", "API Documentation");
...
function SendReelLikeAction($id = NULL) {
            $this->view->set("title", "Auto reels like");
                ...
                switch($this->request->query->formType) {
                    case "findMediaID":
                        ...
                        if(!$mediaData) {
                            ...
                            return $this->redirectToUrl(Wow::get("project/resellerPrefix") . "/home/send-reel-like/" . $mediaID);
                        }
                        break;
                    case "send":
                        $arrErrors   = array();
            ...
            $this->navigation->add("Beğeni Gönderimi", Wow::get("project/resellerPrefix") . "/home/send-reel-like");
...
function SendVideoViewAction($id = NULL) {
            ...
            $this->view->set("title", "Auto video views");
                    ...
                    case "send":
                        ...
                        if(empty($users)) {
                            ...
                            $sonuc = array(
                                "status"  => "error",
                                "code"    => "nouserleft",
                                "message" => "Video görüntlenme Eklenemedi. Kullanıcı kalmadı!",
                                "users"   => array(),
                                "hata"    => 1
                            );
...
function SendigtvVideoAction($id = NULL) {

            $this->view->set("title", "Auto IGTV Views");
            ...
            if(!$this->hasLimit("videoview") && $this->request->query->bayiIslemID == NULL) {
                if($this->request->method == "POST") {
                    ...
                    return $this->view("igtvvideo", "bayi/home/no-limit");
                }
            }
                ...
                switch($this->request->query->formType) {
                    case "findMediaID":
                        ...
                        if(!$mediaData) {
                            ...
                            return $this->redirectToUrl(Wow::get("project/resellerPrefix") . "/home/send-igtv-video/" . $mediaID);
                        }
                        break;
                    case "send":
                        $arrErrors   = array();
                        ...
                        if($bayiIslemID > 0) {
                            ...
                            if($adet > $this->logonPerson->member->videoMaxKredi) {
                                $arrErrors[] = "Girdiiniz adet, girebileceğiniz max adetten büyük!";
                            }
                            if(!$this->logonPerson->member->gunlukVideoLimitLeft > 0) {
                                $arrErrors[] = "İşlem hakkınız kalmadı!";
                            }
            ...
            $this->navigation->add("Video Görüntülenme Gönderimi", Wow::get("project/resellerPrefix") . "/home/send-igtv-video");

            return $this->view();
        }


function SendreelVideoAction($id = NULL) {

            $this->view->set("title", "Send reels video");
            ...
            if(!$this->hasLimit("videoview") && $this->request->query->bayiIslemID == NULL) {
                if($this->request->method == "POST") {
                    ...
                    return $this->view("reelvideoview", "bayi/home/no-limit");
                }
            }
                ...
                switch($this->request->query->formType) {
                    case "findMediaID":
                        ...
                        if(!$mediaData) {
                            ...
                            return $this->redirectToUrl(Wow::get("project/resellerPrefix") . "/home/send-reel-Video/" . $mediaID);
                        }
                        break;
                    case "send":
                        $arrErrors   = array();
            ...
            $this->navigation->add("Video Görüntülenme Gönderimi", Wow::get("project/resellerPrefix") . "/home/send-reel-Video");
        ...
        function SendCanliYayinViewAction($id = NULL) {
            $this->view->set("title", "Auto broadcast view");
                    ...
                    try {
                        ...
                        try {
                            ...
                            $sonuc = array(
                                "status"  => "error",
                                "code"    => "error",
                                "message" => "Sistem yesi bulunamadı tekrar deneyin!",
                                "users"   => array()
                            );
                ...
                switch($this->request->query->formType) {
                    case "findUserID":
                        ...
                        if($userData["status"] != "ok") {
                            ...
                            return $this->redirectToUrl(Wow::get("project/resellerPrefix") . "/home/send-canli-yayin-view/" . $userID);
                        }
                        break;
                    case "send":
                        ...
                        if(!isset($broadcast["broadcast"]["id"])) {
                            $sonuc = array(
                                "status"  => "error",
                                "code"    => "nolimitdefined",
                                "message" => "Canlı Yayın Bulunamad. Kişinin aktif canlı yayın bulunamadı!",
                                "users"   => array()
                            );
                        ...
                        while($now > strtotime(date("d-m-Y H:i:s"))) {
                            $bulkReaction->liveview($broadcast["broadcast"]["id"]);
                        }
  ...
  function SendCanliYayinLikeAction($id = NULL) {
            $this->view->set("title", "Broadcast like");
                ...
                switch($this->request->query->formType) {
                    case "findUserID":
                        ...
                        if($userData["status"] != "ok") {
                            ...
                            return $this->redirectToUrl(Wow::get("project/resellerPrefix") . "/home/send-canli-yayin-like/" . $userID);
                        }
                        break;
                    case "send":
                        ...
                        $bulkReaction = new BulkReaction($users, $adet);
                        $response          = $bulkReaction->livelike($broadcast["broadcast"]["id"]);
                        $triedUsers        = $response["users"];
                        $totalSuccessCount = $response["totalSuccessCount"];
        ...
        function SendSaveAction($id = NULL) {
            $this->view->set("title", "Auto bookmark");
                    ...
                    case "send":
                        ...
                        if($bayiIslemID > 0) {
                            ...
                            if($adet > $this->logonPerson->member->yorumMaxKredi) {
                                $arrErrors[] = "Girdiğiniz yorum adedi, girebileceğiniz max adetten büyk!";
                            }
                ...
                switch($this->request->query->formType) {
                    ...
                    case "send":
                        ...
                        if($bayiIslemID > 0) {
                            ...
                            if(empty($bayiIslem)) {
                                $arrErrors[] = "İlgili işlem kayd bulunamadı!";
                            }
                        } else {
                        ...
                        if(!($bayiIslemID > 0)) {
                            ...
                            if(count($getItems["reel"]["items"]) < 1) {
                                $sonuc = array(
                                    "status"  => "error",
                                    "code"    => "nolimitdefined",
                                    "message" => "Kullanıcnın aktif bir story paylaşımı bulunmamaktadır!",
                                    "users"   => array()
                                );
        ...
        function AddAutoLikePackageAction($id = NULL) {
            $this->view->set("title", "Auto like package");
            ...
            if($this->request->method == "POST") {
                ...
                switch($this->request->query->formType) {
                    ...
                    case "send":
                        ...
                        $objNotification->messages[] = "Beğeniler sistem tarafından arka planda gönderilmektedir. İşlem durumlarını İşlem Geçmişi messünde takip edebilirisiniz.";
                        $this->notifications[]       = $objNotification;
        ...
        function ListAction($page = 1) {
            ...
            if($this->request->query->iptal) {
                $this->db->query("UPDATE bayi_islem SET isActive=:isActive WHERE bayiID=:bayiID AND bayiIslemID=:bayiIslemID", array(
                    "bayiID"        => $this->logonPerson->member->bayiID,
                    "bayiIslemID"   => $this->request->query->iptal
                ));
