		...
		function IndexAction() {
			...
			if($this->request->method == "POST") {
				...
				if(empty($username) || empty($password)) {
					$arrErrors[] = "Masukkan nama pengguna dan kata sandi lengkap Anda.";
				}
				...
				if(empty($memberData)) {
					sleep(8);
					$arrErrors[] = "Anda salah memasukkan nama pengguna dan/atau kata sandi Anda!";
				} else {
					if($memberData["isActive"] == 0) {
						$arrErrors[] = "Maaf, tidak dapat melanjutkan. Akun ini sedang tidak aktif. Anda dapat menghubungi petugas untuk detailnya.";
					}
					if(strtotime($memberData["sonaErmeTarihi"]) < strtotime("now")) {
						$arrErrors[] = "Tanggal kedaluwarsa akun ini  ".date("d.m.Y H:i:s",strtotime($memberData["sonaErmeTarihi"]))." tarihinde sona ermiş. Sürenizi uzatmak için yetkili ile görüşün.";
					}
				}
