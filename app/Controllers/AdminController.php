    ...
    class AdminController extends BaseController {
        ...
        function SettingsAction() {
            if($this->request->method == "POST") {
                ...
                $objNotification->title      = "Perubahan tersimpan.";
                $objNotification->messages[] = "Perubahan Anda pada pengaturan disimpan.";
            ...
            if($this->request->method == "POST") {
                switch($this->request->query->formType) {
                    ...
                    case "addUserPass":
                        ...
                        $sonuc       = array(
                            "status"   => "error",
                            "message"  => "pengguna: kata sandi salah",
                            "instaID"  => "0",
                            "userNick" => ""
                        );
                            ...
                            try {
                                ...
                                if($l["status"] == "ok") {
                                    ...
                                    if(empty($uyeID)) {
                                        ...
                                        $sonuc = array(
                                            "status"   => "error",
                                            "message"  => "Pengguna Sudah Ditambahkan",
                                            "instaID"  => $instaID,
                                            "userNick" => $username
                                        );
                                    }
            ...
            if($this->request->method == "POST") {
                switch($this->request->query->formType) {
                    case "saveUserDetails":
                        ...
                        $objNotification             = new Notification();
                        $objNotification->title      = "Perubahan tersimpan.";
                        $objNotification->messages[] = $user["kullaniciAdi"] . " Perubahan yang Anda buat pada pengguna disimpan.";
                        $objNotification->type       = $objNotification::PARAM_TYPE_SUCCESS;
                        $this->notifications[]       = $objNotification;
                    ...
                    case "addPackage":
                        ...
                        $objNotification->messages[] = $user["kullaniciAdi"] . " Paket ditentukan untuk pengguna.";
                        $objNotification->type       = $objNotification::PARAM_TYPE_SUCCESS;
                        $this->notifications[]       = $objNotification;
                    ...
                    case "updatePackage":
                        ...
                        $objNotification             = new Notification();
                        $objNotification->title      = "Perubahan Paket Disimpan.";
                        $objNotification->messages[] = "Perubahan Anda pada paket telah disimpan.";
                        $objNotification->type       = $objNotification::PARAM_TYPE_SUCCESS;
                        $this->notifications[]       = $objNotification;
        ...
        function SayfalarAction() {
            ...
            $this->navigation->add("Pages", "/admin/sayfalarr");
        ...
        function SayfaDuzenleAction($id) {
            ...
            if($this->request->method == "POST") {
                ...
                $objNotification->type       = $objNotification::PARAM_TYPE_SUCCESS;
                $objNotification->title      = "Perubahan tersimpan.";
                $objNotification->messages[] = "Perubahan Anda pada halaman telah disimpan.";
                $this->notifications[]       = $objNotification;
        ...
        function BlogAction() {
            ...
            if($this->request->method == "POST") {
                ...
                if(intval($lastInsertID) > 0) {
                    $objNotification             = new Notification();
                    $objNotification->type       = $objNotification::PARAM_TYPE_SUCCESS;
                    $objNotification->title      = "Konten Blog Baru Disimpan.";
                    $objNotification->messages[] = "Anda dapat melengkapi bagian yang hilang di bawah ini.";
                    $this->notifications[]       = $objNotification;
            ...
            if(intval($this->request->query->deleteBlogID) > 0) {
                ...
                if($rowsAffected > 0) {
                    ...
                    $objNotification->title      = "Konten Blog Dihapus.";
                    $objNotification->messages[] = "Blog yang ingin Anda hapus telah berhasil dihapus. ";
                    $this->notifications[]       = $objNotification;
        ...
        function BlogDuzenleAction($id) {
            ...
            if($this->request->method == "POST") {
                ...
                $objNotification->type       = $objNotification::PARAM_TYPE_SUCCESS;
                $objNotification->title      = "Perubahan tersimpan.";
                $objNotification->messages[] = "Perubahan Anda pada konten blog disimpan.";
                $this->notifications[]       = $objNotification;
            ...
            if($this->request->method == "POST") {
                switch($this->request->query->formType) {
                    case "saveBayiDetails":
                        ...
                        $objNotification             = new Notification();
                        $objNotification->title      = "Perubahan tersimpan.";
                        $objNotification->messages[] = $bayi["username"] . " Perubahan yang Anda buat di dealer akan disimpan.";
                        $objNotification->type       = $objNotification::PARAM_TYPE_SUCCESS;
                        $this->notifications[]       = $objNotification;
            ...
            if($this->request->method == "POST") {
                switch($this->request->query->formType) {
                    case "saveBayiEkle":
                        ...
                        $objNotification             = new Notification();
                        $objNotification->title      = "Reseller Baru Ditambahkan. ";
                        $objNotification->messages[] = $this->request->data->username . " Dealer telah berhasil ditambahkan.";
                        $objNotification->type       = $objNotification::PARAM_TYPE_SUCCESS;
                        $this->notifications[]       = $objNotification;
