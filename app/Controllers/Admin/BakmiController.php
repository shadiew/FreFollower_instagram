...
class BakmiController extends BaseController
{
        ...
        if ($this->request->method == "POST") {
            switch ($this->request->query->formType) {
                case "saveUserDetails":
                    ...
                    $objNotification->messages[] = $user["kullaniciAdi"] . " Perubahan yang Anda buat sudah tersimpan.";
    ...
    function UyeSilAction($id)
    {
        ...
        $objNotification             = new Notification();
        $objNotification->title      = "Member telah dihapus.";
        $objNotification->messages[] = $uye["kullaniciAdi"] . " Member telah dihapus.";
        $objNotification->type       = $objNotification::PARAM_TYPE_DANGER;
        $this->notifications[]       = $objNotification;

        return $this->redirectToUrl(Wow::get("project/adminPrefix") . "/bakmi");
    }
}
