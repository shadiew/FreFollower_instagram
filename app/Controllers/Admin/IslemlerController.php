<?php
    ...
    class IslemlerController extends BaseController {
        ...
        function IndexAction() {
            ...
            if($this->request->method == "POST") {
                switch($this->request->query->formType) {
                    case "removePassiveUsers":
                        ...
                        break;
                    case "removeNAUsers":
                        $passiveUsers = $this->db->query("SELECT instaID,kullaniciAdi FROM uye WHERE isActive=99");
                        foreach($passiveUsers as $pUser) {
                            $filePath = Wow::get("project/cookiePath") . "instagramv3/" . substr($pUser["instaID"], -1) . "/" . $pUser["instaID"] . ".iwb";
                            if(file_exists($filePath)) {
                                unlink($filePath);
                        ...
                        $this->db->query("DELETE FROM uye WHERE isActive=99");
            ...
            $data["countNaUsers"]      = $this->db->single("SELECT COUNT(*) FROM uye WHERE isActive=99");
            $data["listNaUsers"]       = $this->db->query("SELECT CONCAT(kullaniciAdi,':',sifre) AS userpass FROM uye WHERE isActive=99 OR isActive=0 ORDER BY uyeID DESC");
            ...
            if($this->request->method == "POST") {
                switch($this->request->query->formType) {
                    case "addUserPass":
                        ...
                        $sonuc       = array(
                            "status"   => "error",
                            "message"  => "User: Password is sent faulty",
                            "instaID"  => "0",
                            "userNick" => ""
                        );
                        if(count($expUserpass) > 1) {
                            ...
                            if($userData["status"] != "ok") {
                                ...
                                if($l["status"] == "ok" && !isset($l["step_name"])) {
                                    ...
                                    $userInfo = $i->getSelfUserInfo($userData["user"]["pk"]);
                                    ...
                                    if(empty($uyeID)) {
                                        ...
                                        $sonuc = array(
                                            "status"   => "success",
                                            "message"  => "The Login has been renewed.The user has been activated.",
                                            "instaID"  => $instaID,
                                            "userNick" => $username,
                                            "userData" => $userData
                                        );
                                    ...
                                    if(isset($l["step_name"])) {
                                        $sonuc = array(
                                            "status"   => "error",
                                            "message"  => "Fallen account of approval!",
                                            "instaID"  => "0",
                                            "userNick" => $username
                                        );
                                    } else {
